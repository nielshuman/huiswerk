...
for P=1:size(P2,1)
    preprocessed_filename=[DRIVE '\data\sound_response' num2str(P2(P))   '.mat'];
...
for P=1:size(P2,1)
    ...
    if ~isempty(sorted_data{P}.good_sorts{1})
        ...
        if plot_autocorrelation
            subplot(4,size(P2,1),P+size(P2,1))
        ...
        if do_PCA
            ...
            subplot(4,size(P2,1),P+size(P2,1)+size(P2,1))
        ...
        subplot(4,size(P2,1),P+size(P2,1)+size(P2,1)+size(P2,1)),cla
         plot(sorted_data{P}.trig_resp_bins,sorted_data{P}.trig_resp,'k-','linewidth',2); hold on
         axis tight; box off; xlabel('time'); ylabel('sp/s');
...
saveas(gcf,[DRIVE '/results/Results_part4.png'])
