...
load([data_folder 'In_OR_OUT_data2'],'example_data','test_data')
...
for D=1:2
    ...
    set(gca,'ZTickLabel',['Play sound ' num2str(D)],'xcolor','k','ycolor','k','linewidth',3,'fontsize',16,'ylim',[-0.01 0.01]), box off;
...
%
% DRIVE='F:\data\PD_threshold_2018_nov';
% N= [14]
% save_filename=[DRIVE '\Patient_' num2str(N) '_save_data.mat'];
% load(save_filename)
%
% K=1
% Ds=[1 10]%1:length(save_data.depths);
% for D=1:2
%     all_file=[DRIVE '\Patient_' num2str(N) '_preprocessed_data_site' num2str(Ds(D)) '_CH' num2str(K)   '.mat'];
%     if exist(all_file)==2
%         load(all_file);
%         if isfield(preprocessed_data,'t')
%             t=preprocessed_data.t(1):preprocessed_data.t(2):preprocessed_data.t(3);
%             ix=find(t>5 & t<15);
%             example_data{D}.t=t(ix);
%             example_data{D}.d=preprocessed_data.Data(ix);
%             %example_data{D}.p=audioplayer(preprocessed_data.Data*100,preprocessed_data.SampFreq);
%             example_data{1}.fs(D)=preprocessed_data.SampFreq;
%         end
%     end
% end
%
...
% Ks=[ 1  2  3  1  2  3];
% for D=1:length(N)
%     all_file=[DRIVE '\Patient_' num2str(N(D)) '_preprocessed_data_site' num2str(Ds(D)) '_CH' num2str(K)   '.mat'];
%     if exist(all_file)==2
%         load(all_file);
%         if isfield(preprocessed_data,'t')
%             t=preprocessed_data.t(1):preprocessed_data.t(2):preprocessed_data.t(3);
...
%
%             test_data{D}.t=t(ix);
%             test_data{D}.d=preprocessed_data.Data(ix);
%
%             test_data{1}.correct(D)=0;
%             test_data{1}.fs(D)=preprocessed_data.SampFreq;
%         end
%     end
% end
% count=D;
...
%             ix=find(t>5 & t<15);
%
%             test_data{D+count}.t=t(ix);
%             test_data{D+count}.d=preprocessed_data.Data(ix);
%
%             test_data{1}.correct(D+count)=1;
%             test_data{1}.fs(D+count)=preprocessed_data.SampFreq;
%         end
%     end
% end
% save([data_directory 'In_OR_OUT_data2'],'example_data','test_data')
% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
